...
// g++ -std=c++11 -Wall tree-main.cpp string-utility.cpp tree-chsib.cpp list-array.cpp -o tree
...
//FUNZIONE AUSILIARIA PER TROVARE UN NODO E RESTITUIRNE IL PUNTATORE AD ESSO
treeNode* findNode(Label l, Tree t) {
    if (isEmpty(t)) return emptyTree;
    if (t->label == l) return t;

    //scorro i figli
    treeNode* res = findNode(l, t->firstChild);
    if (res != nullptr) return res;

    //scorro i fratelli
    return findNode(l, t->nextSibling);
}
...
Error tree::addElem(const Label labelOfNodeInTree, const Label labelOfNodeToAdd, Tree& t)
{
   ...
   else
   {
      ...
      treeNode* nuovo = new treeNode;
      nuovo->label = labelOfNodeToAdd;
      nuovo->firstChild = emptyTree;

      nuovo->nextSibling = padre->firstChild; 
      
      padre->firstChild = nuovo;
...
list::List tree::children(const Label l, const Tree& t)
{
   ...
   Tree fatherNode = findNode(l, t);
   if (fatherNode != emptyTree)
   {
      Tree childNode = fatherNode->firstChild;
      while (childNode != emptyTree)
      {
         addBack(childNode->label, lst);
