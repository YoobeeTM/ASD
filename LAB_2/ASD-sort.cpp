...
int partitionTrivial(vector<int>& v, int low, int high) {
    int pivot = v.at(high);  // pivot = ultimo elemento
    int i = low - 1;
...
void quickSortTrivialRec(vector<int>& v, int low, int high) {
    if (low < high) {
        int pi = partitionTrivial(v, low, high);
        quickSortTrivialRec(v, low, pi - 1);
        quickSortTrivialRec(v, pi + 1, high);
    }
}
...
int partitionRandom(vector<int>& v, int low, int high) {
    int randomIndex = low + rand() % (high - low + 1);
    swap_ind(v, randomIndex, high);  // porta pivot in fondo
    return partitionTrivial(v, low, high);
}

void quickSortRandomRec(vector<int>& v, int low, int high) {
    if (low < high) {
        int pi = partitionRandom(v, low, high);
        quickSortRandomRec(v, low, pi - 1);
        quickSortRandomRec(v, pi + 1, high);
    }
}

void quickSortRandom(vector<int>& v){
    srand(time(0));  // inizializza random
